...
class Tile {
  private x: number = 0;
  private y: number = 0;
  private player: string = notPlayed;
  ...
  constructor() {
    for (let x = firstRow; x <= thirdRow; x++) {
      for (let y = firstColumn; y <= thirdColumn; y++) {
        this._plays.push(new Tile(x, y, notPlayed));
      }
    }
  }
